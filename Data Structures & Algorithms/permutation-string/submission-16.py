    ...
    def checkInclusion(self, s1: str, s2: str) -> bool:
        ...
        for r in range(len(s1), len(s2)):
            ...
            if s2Count[right] == s1Count[right]:
                matches += 1
            #At most, after each increment s2Count at right will differ from s1Count by 1, if it does decrement matches
            elif s2Count[right] == s1Count[right]+1: 
                matches -= 1
            
            #Sliding Window: Decrement and Continously Move the Window relative to R to keep the window len equal to len(s1)
            left = ord(s2[l]) - ord('a') 
            s2Count[left] -= 1
